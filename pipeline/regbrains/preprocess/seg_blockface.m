function varargout = seg_blockface(varargin)
% SEG_BLOCKFACE MATLAB code for seg_blockface.fig
%      SEG_BLOCKFACE, by itself, creates a new SEG_BLOCKFACE or raises the existing
%      singleton*.
%
%      H = SEG_BLOCKFACE returns the handle to a new SEG_BLOCKFACE or the handle to
%      the existing singleton*.
%
%      SEG_BLOCKFACE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEG_BLOCKFACE.M with the given input arguments.
%
%      SEG_BLOCKFACE('Property','Value',...) creates a new SEG_BLOCKFACE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before seg_blockface_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to seg_blockface_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help seg_blockface
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @seg_blockface_OpeningFcn, ...
                   'gui_OutputFcn',  @seg_blockface_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before seg_blockface is made visible.
function seg_blockface_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to seg_blockface (see VARARGIN)

% Choose default command line output for seg_blockface
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes seg_blockface wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = seg_blockface_OutputFcn(hObject, eventdata, handles) 
...
global root_dir block_dir orig_dir seg_dir init_file init_str tmp_dir;
...
block_dir = strcat(root_dir,'blockface/');
orig_dir = strcat(block_dir, 'orig/');
seg_dir = strcat(block_dir,'seg/');
init_file = strcat(block_dir,'init_segmentation.mat'); 
try
    init = load(init_file);
...
preprocess_seg_block_v2(root_dir,init_str.wp,init_str.tissue,init_str.back,'jpg');
