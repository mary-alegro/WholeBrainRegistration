function [trans,uv,xy,uv_dev,xy_dev] = cp2tform_mod(varargin)
...
%   $Revision: 1.10.4.23 $  $Date: 2011/08/09 17:49:26 $
...
switch method
  ...
  case 'lwm'
    findT_fcn = @findLWM;
  case 'nonreflective similarity noscaling'
    findT_fcn = @findNonreflectiveSimilarityNoScaling;
  otherwise
...
if rank(X) >= 2*K 
    ...
    error(message('images:cp2tform:rankError', K, 'projective'))
end
...
function [trans, output] = findNonreflectiveSimilarityNoScaling(uv,xy,options)
%
% For a nonreflective similarity:
%
% let sc = s*cos(theta)
...
%find only cos(tetha) without the scaling factor
S = sqrt((ss^2)+(sc^2));
sc = sc/S;
ss = ss/S;
...
if rank(X)>=K
    ...
    error(message('images:cp2tform:rankError', K, 'polynomial'))
end
...
if size(xy,2) ~= 2
    error(message('images:cp2tform:xyMustHave2Cols'))
end

if size(xy,1) < 1
    error(message('images:cp2tform:xyMustHaveOneRow'))
end

if size(t.tdata,2) ~= 2
    error(message('images:cp2tform:tdataMustHave2Cols'))
end
...
switch nterms
  ...
  otherwise
    error(message('images:cp2tform:tdataInvalidNumOfRows'))
end
...
switch order
  ...
  otherwise
    error(message('images:cp2tform:invalidOrder'))
end
...
if (ntri<2)
    error(message('images:cp2tform:rankError', options.K, 'piecewise linear'))
end
...
if ~isempty(bad_triangles)
    ...
    if (nvert < options.K)
        error(message('images:cp2tform:countError', num_bad_vertices, nvert, options.K, 'piecewise linear'))
    end
    x = xy(:,1);
    y = xy(:,2);
    tri = delaunay(x,y);
    ...
    if ~isempty(more_bad_triangles)
        error(message('images:cp2tform:foldoverTrianglesRemain', num_bad_vertices))
    end
    
    % print report about triangles and how many points were eliminated
    warning(message('images:cp2tform:foldoverTriangles', sprintf( '%d ', bad_triangles ), sprintf( '%d ', bad_vertices )))
end
...
switch nargs_to_go
  ...
  case 1
    ...
    elseif strcmp(method,'lwm')
        ...
        error(message('images:cp2tform:tooManyInputs'))
    end
...
switch method
  ...
  case 'polynomial'
    ...
    if ~isnumeric(order) || numel(order)~=1 || sum(order==2:4)~=1
        error(message('images:cp2tform:invalidPolynomialOrder'))
    end
...
methods = {'affine',...
           ...
           'nonreflective similarity noscaling',...
