function register_histo_block2(directory,svref)
...
reg_dir = strcat(directory,'/reg_block2/');
...
%propagate to othes blocks from top to bottom
    ref_img_name = blocks(1).files(end).name;
    ref_img_name = strcat(reg_dir,ref_img_name);
    ref_img = double(imread(ref_img_name));
    
    %ref_img_name = blocks(1).files(end-1).name;
    %ref_img_name = strcat(reg_dir,ref_img_name);
    %ref_img2 = double(imread(ref_img_name));
    
    %  includes reference image and its upper neighboor
    %ref_img = cat(3,ref_img,ref_img2);
...
%propagate to othes blocks from bottom to top
